...
export const PATCH = async (request: Request, context: { params: any }) => {
  ...
  const categoryId = context.params.category;  // This is taken from the parent & dynamic [category] folder name
  ...
  try {
    ...
    const userId = searchParams.get("userId"); // userId taken from URL (?userId=.......)
...
export const DELETE = async (request: Request, context: { params: any }) => {
  ...
  try {
    ...
    if (!category) {
      return new NextResponse(
        JSON.stringify({ message: "Category not found or does not belong to user" }), { status: 404 }
      );
    }
